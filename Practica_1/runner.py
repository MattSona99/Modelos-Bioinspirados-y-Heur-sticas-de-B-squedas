...
from Practica_1.config import CASOS, SEMILLAS_P1, SEMILLAS_P2, TOLERANCIA
...
from Practica_2.algorithms import (
    busqueda_grasp,
    busqueda_ils,
    busqueda_vns
)
...
REGISTRY = {
    ...
    'grasp': {
        'nombre_display': 'GRASP (Multiarranque)',
        'funcion': busqueda_grasp,
        'is_deterministic': False
    },
    'ils': {
        'nombre_display': 'ILS',
        'funcion': busqueda_ils,
        'is_deterministic': False
    },
    'vns': {
        'nombre_display': 'VNS',
        'funcion': busqueda_vns,
        'is_deterministic': False
    }
}
...
def ejecutar_experimento(
    ...
    **kwargs
    ...
    # Cargar datos base
    coordenadas = cargar_coordenadas(os.path.join(directorio_runner, 'coords.json'))
    ...
    for nombre_caso, datos in casos.items():
        ...
        # Fila de la tabla
        fila = f"| {nombre_caso:<6} | {mejor_res_absoluto['score_universal']:>11.4f} | {mejor_res_absoluto['kms']:>7.2f} | {mejor_res_absoluto['entropia']:>8.4f} | {ev_media:>8.1f} | {ev_mejor:>9} | {t_medio:>12.4f} | {semilla_str:>17} | {nombre_fobj_corta:<10} |"
